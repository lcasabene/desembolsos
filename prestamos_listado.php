...
header('Location: menu_bienes_moderno.php');
exit;
...
// Traer préstamos (LÓGICA CORREGIDA PARA PDO)
$params = []; // Inicializamos array de parámetros
...
if ($isAdmin) {
    ...
    // Si es admin, no hay parámetros con '?' en la query
} else {
    $sql = "SELECT p.*, u.nombre AS solicitante
            FROM prestamos_bienes p
            LEFT JOIN usuarios u ON u.id = p.usuario_solicitante_id
            ...
            ORDER BY p.fecha_solicitud DESC";
    // Si es usuario, añadimos el ID al array de parámetros
    $params[] = $user_id;
}

$stmt = $pdo->prepare($sql);
$stmt->execute($params); // Pasamos los parámetros aquí (si los hay)
...
<div class="container mt-4">
   ...
   <div class="d-flex justify-content-end gap-2 mb-3">
    ...
    <a href="prestamos_form.php" class="btn btn-success">
